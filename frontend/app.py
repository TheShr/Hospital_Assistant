...
with st.sidebar:
    ...
    with col_r:
        if st.button("↺ Refresh", use_container_width=True):
            ...
            st.rerun()
    with col_all:
        if st.button("All docs", use_container_width=True):
    ...
    if st.session_state.docs_list:
        for doc in st.session_state.docs_list:
            label = doc["filename"][:28] + ("…" if len(doc["filename"]) > 28 else "")
            is_active = st.session_state.current_doc_id == doc["id"]
            prefix = "✅ " if is_active else "📄 "
            if st.button(prefix + label, key=f"doc_{doc['id']}", use_container_width=True):
...
<div class="hero">
    ...
    <div class="hero-text">
        ...
        <p>AI-powered patient query system · Answers strictly from your hospital document · Zero hallucination</p>
    </div>
</div>
""", unsafe_allow_html=True)
